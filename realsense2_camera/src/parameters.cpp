...
void BaseRealSenseNode::getParameters()
{
    ...
    param_name = std::string("camera_namespace");
    _camera_namespace = _parameters->setParam<std::string>(param_name, "");
    ...
    _base_frame_id = (static_cast<std::ostringstream&&>(std::ostringstream() << (_camera_namespace.empty() ? "" : _camera_namespace + "/") <<_camera_name << "_" << _base_frame_id)).str();
    _parameters_names.push_back(param_name);
